...
# outGSA <- read_csv(file = "output/GSA_output/GSA_output_PRCC_2023-12-16.csv") %>% select(-c("...1))
...
# read in valid pars for pairs plot
# parsGSA.valid <- read_csv(file = "output/GSA_output/GSA_parsValid_PRCC_2023-12-16.csv") %>%
#   select("offtake_m1" = off_mY, "offtake_m2"=off_mA, "offtake_f"=off_f, "mortality_y"=mort_Y, "mortality_a"=mort_A, birth_rate, "age_exchange"=min_off, "age_reprod"=min_repro, "age_m"=max_yrs_M, "age_f"=max_yrs_F)
...
if(plotsave){
  ...
  write.csv(outGSA.valid %>% select(-c(`...1`,set, tenyr_15, tenyr_15age)), file = "output/GSA_output/GSA_outputValid_PRCC_2023-12-16.csv", row.names = F)
   write.csv(parsGSA.valid %>% select(-c(tenyr_growth)), file = "output/GSA_output/GSA_parsValid_PRCC_2023-12-16.csv", row.names = F)
}


# ### Correlation plot - monotonicity:
# monoton_df <- parsGSA.valid %>%
#   gather(key = "Par", 
#          value = "value", 
#          -c(tenyr_growth)) %>%
#   mutate(Par = as.factor(Par))
# 
# monoton_plot <- ggplot(monoton_df, aes(x=value, y = log(tenyr_growth)))+
#   geom_point()+
#   facet_wrap(~Par, scales = "free_x")+
#   labs(x = "Parameter value", y = "Ten-year growth rate (log-scale)")

ggsave(monoton_plot, file = "plots/for-methods/input-output-monotonicity-check.png", )
```
...
prccGSA.valid <- parsGSA.valid %>%
  select("offtake_mY" = off_mY, 
         "offtake_mA"=off_mA, 
         "offtake_f"=off_f, 
         "mortality_y"=mort_Y, 
         "mortality_a"=mort_A, 
         birth_rate, 
         "age_exchange"=min_off, 
         "age_reprod"=min_repro, 
         "age_m"=max_yrs_M, 
         "age_f"=max_yrs_F, 
         tenyr_growth) %>%
...
prccb.all <- rbind(prccb_imm6m %>% select(var,est) %>% mutate(output = "imm_6M"),
                  prccb_imm12m %>% select(var,est) %>% mutate(output = "imm_12M"),
                  prccb_imm70 %>% select(var,est) %>% mutate(output = "imm_70%"), 
                  prccb_growth %>% select(var,est) %>% mutate(output = "pop_growth")) %>%
  mutate(output = factor(output, levels = c("imm_6M", "imm_12M", "imm_70%", "pop_growth")))
