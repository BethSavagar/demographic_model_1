...
agesex.long <- agesex.comparison %>%
  ...
  mutate(age_group = factor(age_group, levels = c("pfAdu", "pfSub","pfKid","pmAdu","pmSub","pmKid","pF")))
...
ggplot(agesex.long %>% filter(age_group %in% c("pfAdu", "pfSub","pfKid"), 
                              id == "seasonal"),
       aes(x=w,y=prop))+
...
custom_palette <- c("#004D40", "#00695C", "#00796B", "#00897B", "#009688", "#4A148C", "#6A1B9A", "#7B1FA2", "#8E24AA", "#9C27B0")
custom_palette <- c(custom_palette, rep("#000000", 10))
