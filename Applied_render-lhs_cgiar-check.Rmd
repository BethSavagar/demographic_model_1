...
# datafile <- "-cgiar_check2" # rerun based on updated ranges comparing with RSA
...
ranges_cgiar <- ggarrange(p1,p2)
filename_ranges <- "ranges_cgiar.png"

if(plotsave){
  ggsave(paste0("plots/",filename_ranges), plot = ranges_cgiar, width = 20, height = 10, units = "cm")
}
...
dynamicsList <- lapply(cgiarOut.dynamics, function(x){
 do.call(rbind,x) %>% mutate(set = rep(1:50, each = TimeStop_dynamics))
  }) 
  
Out_dynamics <- dynamicsList %>%
  do.call(rbind, .) %>%
  mutate(prof = rep(datasets, each = 52000))
...
# Age-Sex Plot:
agesex_dynplotG <- ggplot(Out_agesex %>% filter(prof=="cgiar.goat"), 
       aes(x = w, y = value, group = set, col = par)) + 
  geom_line()+
  facet_wrap(~par)+
  labs(x="Weeks", 
       y = "Proportion",
       title = "Age-Sex dynamics, cgiar Goat, (LHS Parameter Sets)")+
  theme_bw()

agesex_dynplotSh <- ggplot(Out_agesex %>% filter(prof=="cgiar.shp"), 
       aes(x = w, y = value, group = set, col = par)) + 
  geom_line()+
  facet_wrap(~par)+
  labs(x="Weeks", 
       y = "Proportion",
       title = "Age-Sex dynamics, cgiar Sheep, (LHS Parameter Sets)")+
  theme_bw()
...
agesex_dynShG <- ggarrange(agesex_dynplotSh,agesex_dynplotG, ncol = 1)

filename_growthDyn <- "growthDyn_cgiarShG.png"
filename_agesexDyn <- "agesexDyn_cgiarShG.png"

if(plotsave){
  ggsave(paste0("plots/",filename_growthDyn), plot = growth_dynplot, width = 20, height = 10, units = "cm")
  ggsave(paste0("plots/",filename_agesexDyn), plot = agesex_dynShG, width = 20, height = 20, units = "cm")
}
```
...
summaryList <- cgiarOut.summary
cgiarOut.summary <- summaryList %>%
  do.call(rbind, .) %>%
...
popgrowth <- cgiarOut.summary %>% 
  select(prof, ends_with("growth")) %>% 
  gather(key = "par", value = "prop", -prof) %>% 
...
growth_boxplot <- ggplot(popgrowth, aes(x=prof, y = prop, col = prof))+
  ...
  labs(x = "Growth Stat", 
       y = "Pop Growth",
       title = "Population growth, cgiar, LHS parameter sets")+
  theme_bw()
...
agesex <- cgiarOut.summary %>% 
  select(prof, starts_with("pf"), starts_with("pm")) %>% 
  gather(key = "par", value = "prop", -prof)
...
summary_ShG <- ggarrange(growth_boxplot, growth_hist, agesex_boxplot, ncol = 1)
filename_summary <- "summary_cgiarShG.png"

if(plotsave){
  ggsave(paste0("plots/",filename_summary), plot = summary_ShG, width = 20, height = 20, units = "cm")
}
```
...
cgiarOut.growth %>% group_by(prof, tenyr_15, tenyr_15age) %>% count() %>% kable()

# Growth well in excess of RSA conditions
