...
# use  par sets from LHS validated again tenyr_growth & age-sex (GSA) conditions (11/12/2023)
# - see lhs_OC-check.Rmd for details
...
births.long <- births.clean %>%
  ...
  mutate(weeks = as.numeric(weeks)) %>%
  group_by(prof,set) %>%
  mutate(yr = rep(1:(TimeStop_dynamics/52), each = 52)) %>% 
  ungroup()
...
# calc mean annual births in final ten years of sim
...
kable(births.mean)
filename_nBirths <- "Applied_nBirths_OC100.csv"
if(plotsave){
...
######################################
Out.summary %>% mutate(prof = profOrder) %>% group_by(prof) %>% count()
...
plot_imm <- ggplot(validImm %>% filter(stat %in% c("imm_V0", "imm_6m", "imm_12m")), 
  ...
  geom_hline(yintercept = 0.7, linetype = "dashed")+
  facet_wrap(~stat)+
  labs(x="Profile", y = "Flock proportion immune")+
  theme_bw()+
...
validImm %>% ungroup() %>% filter(stat =="imm70_w") %>% group_by(prof) %>% summarise(min = min(value), 
                                                                           max = max(value), 
                                                                           median = median(value),
                                                                           mean = mean(value))
...
validImm %>% ungroup() %>% group_by(stat) %>% summarise(min = min(value), 
                                                                           max = max(value), 
                                                                           median = median(value),
                                                                           mean = mean(value)) %>% kable()
...
filename_ImmV0612 <- "ImmV0612_OCb.png"
filename_Imm70 <- "ImmSum79_OC.png"
...
ggsave(paste0("plots/applied/",filename_ImmV0612), plot = plot_imm, width = 20, height = 15, units = "cm")
ggsave(paste0("plots/applied/",filename_Imm70), plot = plot_imm70, width = 10, height = 7.5, units = "cm")
