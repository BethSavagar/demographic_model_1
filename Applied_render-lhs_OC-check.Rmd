...
lhs <- T
lhs_n <- 2.5e4 # number of sets (parameter combinations)
pairs_plot <- F
SA <- TRUE
...
```
# Summary stats for 10,000 (25,000) LHS sets per flock profile
...
# valid pars are those which have expected growth but also which have expected mortality rates
ocPars.valid <- ocPars.out %>%
  filter(tenyr_15age == 1, 
         mortality_y>mortality_a,
         NET_offtake_m2>NET_offtake_f,
         NET_offtake_m2>NET_offtake_m)

# filter outvalid to contain same rows as outPars
# ocOut.valid <- ocOut.growth %>%
#   filter(tenyr_15age == 1)

pairs(ocPars.valid %>% select(NET_offtake_y:min_age_repro))
ocPars.valid %>% group_by(prof) %>% count() %>% kable()
...
agesex_filename <- paste0("Applied_AgeSex_OC_",Sys.Date(),".csv")
...
# save as 25k for all validpars from sim with 25,000 sets per profile. And as 100 for just sampled 100 parameter sets
if(plotsave){
...
saveValidPars100 <- saveValidPars %>% group_by(prof) %>% slice_sample(n=100) %>% ungroup()
filename_savePars100 <- paste0("applied_outValidPars100_OC_", Sys.Date(), ".csv")
write.csv(saveValidPars100, file = paste0("output/Applied/", filename_savePars100), row.names = F)
