...
# 
# filename_immDyn_seasonal <- "immDyn_seasonal_A2prof100.csv"
# write.csv(A2dyn_i.clean, file = paste0("output/Applied/", filename_immDyn_seasonal))

```

```{r A2dynamics}
# A2dyn_i.clean <- read_csv("output/Applied/immDyn_seasonal_A2prof100.csv") %>% select(-c("...1"))
...
A2immDynamics.both <- 
  bind_rows(immDynamics.long %>% select(-Vstart),
            A2immDynamics.long %>% select(-Vmin))
  
# Calculate mean and confidence interval
A2summary_data <- A2immDynamics.both %>%
  group_by(pV,prof,weeks,Vweeks,Strategy) %>%
  summarise(
...
plot_immVarAppN2 <- ggplot(A2summary_data %>% filter(Vweeks >=0 & mean_value>0), # so x-axis starts at day 1 of vaccination
  ...
  geom_line(aes(col = factor(pV)), size = 0.7) +
  geom_hline(yintercept = 0.7, linetype = "dashed")+
  facet_grid(Strategy~prof, scales = "free_x")+
...
if(plotsave){
  ...
  # filename_immVarAppN2 <- "OC_seasonalDyn_App100.png"
  # ggsave(paste0("plots/applied/",filename_immVarAppN2), plot = plot_immVarAppN2, width = 15, height = 15, units = "cm")
}
```
...
# filename_immSum_seasonalSumm <- "immDyn_seasonalSumm_A2prof100.csv"
# write.csv(ocOut.seasonalSumm, file = paste0("output/Applied/", filename_immSum_seasonalSumm))
```
...
# A2.seasonalSumm <- read_csv(file = "output/Applied/immDyn_seasonalSumm_A2prof100.csv") %>% select(-c("...1"))
...
if(plotsave){
  ...
  # write_csv(pvir70_summary, file = "tables/applied/seasonal_pvir70_app100.csv")
