...
filename_immDyn_seasonal <- "immDyn_seasonal_prof100.csv"
...
# dyn_i.clean <- read_csv("output/Applied/immDyn_seasonal_prof100.csv") %>% select(-c(`...1`))
...
# filename_immDyn_seasonalSumm <- "immDyn_seasonalSumm_prof100.csv"
...
# ocOut.seasonalSumm <- read_csv("output/Applied/immDyn_seasonalSumm_prof100.csv") %>% select(-c(`...1`))
...
pV.labs <- c("pV=0.7", "pV=0.8", "pV=0.9", "pV=1")
names(pV.labs) <- c(0.7,0.8,0.9,1)
...
plot_imm <- ggplot(validImm %>% filter(stat %in% c("imm_V0", "imm_6m", "imm_12m")),
  ...
  geom_hline(yintercept = 0.7, linetype = "dashed") +
  facet_grid(pV~stat, labeller = labeller(pV = pV.labs)) +
...
filename_imm <- "OC_seasonalImm_100.png"
...
if(plotsave){
  ...
  # write_csv(pvir70_summary, file = "tables/applied/seasonal_pvir70_100.csv")
