...
## Functions:
source(paste0(filepath, "functions/Appliedfunc_seasonal.R")) # wrapper function
...
```{r birthcomparison}
# Verify rate over time is equal between seasonal and non-seasonal (constant birth rate)

births.comparison <- bind_rows(Out.constant %>% select(w,sum_pop) %>% mutate(id = "constant"),
                               Out.seasonal %>% select(w,sum_pop) %>% mutate(id = "seasonal"))
...
ggplot() +
  geom_rect(
   ...
   geom_line(data = births.comparison, aes(x = w, y = sum_pop, col = id), size = 1)+
  labs(x = "weeks (10 year)", 
       y = "population size", 
       col = "birth pattern", 
       title = "Comparison of seasonal vs non-seasonal (constant) birth pattern")

## Zoom in on specific periods to check births implementation
ggplot() +
  geom_rect(
   data = peakrect,
    aes(
   ...
   geom_line(data = births.comparison, aes(x = w, y = sum_pop, col = id), size = 1)+
  geom_point(data = births.comparison, aes(x = w, y = sum_pop, col = id), size = 1)+
  labs(x = "weeks (10 year)", 
       y = "population size", 
       col = "birth pattern", 
       title = "Comparison of seasonal vs non-seasonal (constant) birth pattern")+
  coord_cartesian(xlim=c(415,421))
