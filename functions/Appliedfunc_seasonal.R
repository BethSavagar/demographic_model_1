App_func <- function(
  ...
  ## Immune Fraction ##
  pR <- fix_age_data %>% filter(parameter=="pR") %>% pull(`fixdata`)  # proportion initially in R state
  ## Births in peak month ##
  p_births <- fix_age_data %>% filter(parameter=="pBirths") %>% pull(`fixdata`)  # proportion initially in R state
  ...
  # If seasonal: 
  # set birth rate to low birth rate 
  # # update to peak birth rate for just peak month in the dynmod loop
  # if(seasonal){
  #   birth_r <- birth_rL 
  # }
  ...
  demographic_pars <- data.frame(
    ...
    mutate(net_off_F = ifelse(age_weeks<min_offtake_wks, off_1, off_F),
           ...
           #seasonal births:
           birthL = ifelse(age_weeks<min_repro_wks,0,birth_rL), # birth baseline
           birthH = ifelse(age_weeks<min_repro_wks,0,birth_rH), # birth peak
