...
# ANALYSIS OF cgiar LHS PARAMETER SETS:

cgiar_ranges <- var_demo_data_full %>% 
  select(parameter,
         cgiar.shp.min,
         cgiar.shp.max,
         RSA.min,
...
L1 <- ggplot(cgiar_ranges %>% filter(parameter %in% c("adu_f_max_yrs", "adu_m_max_yrs","min_age_offtake","min_age_repro")), aes(x=parameter, y=value, col = datasource))+
...
L2 <- ggplot(cgiar_ranges %>% filter(!parameter %in% c("adu_f_max_yrs", "adu_m_max_yrs","min_age_offtake","min_age_repro", "ppr_mortality_y", "ppr_mortality_a")), 
...
finyr_summplot <- ggplot(Out2, aes(finyr_growth))+
  ...
  labs(x="Final year growth", 
       y = "Count", 
       title = "Distribution of final year growth, cgiar, (LHS paremeter sets)")+
  theme_bw()
