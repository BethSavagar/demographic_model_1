...
filename_immDyn_nonseasonal12 <- "immDyn_nonseasonalV12.csv" # dec 11 data
filename_immDyn_nonseasonal12 <- "immDyn_nonseasonalV12_prof100.csv" # jan 14 only 100 sets per prof
...
```{r DynamicsAnalysis}
# Load data?
# immDynamics.clean <- read_csv("output/Applied/immDyn_nonseasonalV12.csv") %>% select(-c(`...1`))
# immDynamics.clean <- read_csv("output/Applied/immDyn_nonseasonalV12_prof100.csv") %>% select(-c(`...1`))
...
# filename_immDyn_nonseasonalSumm12 <- "immDyn_nonseasonalSummV12_prof100.csv"
...
filename_immDyn_nonseasonal6 <- "immDyn_nonseasonalV6_prof100.csv"
...
```{r analysisDyn_V6}
# Load data?
# immDynamics6.clean <- read.csv("output/Applied/immDyn_nonseasonalV6.csv", row.names = F)
# immDynamics6.clean <- read.csv("output/Applied/immDyn_nonseasonalV6_prof100.csv", row.names = F)
...
filename_immDyn_nonseasonal6Summ <- "immDyn_nonseasonalSummV6_prof100.csv"
...
```{r combinedanalysisDynamics}
immDynamics.12 <- read_csv("output/Applied/immDyn_nonseasonalV12_prof100.csv") %>% 
  select(-c(`...1`)) %>%
  mutate(Vsched = "Annual")

immDynamics.6 <- read_csv("output/Applied/immDyn_nonseasonalV6_prof100.csv") %>% 
  select(-c(`...1`)) %>%
  mutate(Vsched = "6-monthly")

immDynamics.long <- immDynamics.12 %>%
  bind_rows(immDynamics.6) %>%
  gather(key = "weeks", value = "prop_immune", -c(prof, pV, Vsched)) %>%
  mutate(weeks = as.numeric(weeks)) %>%
  mutate(Vweeks = weeks - 520) %>%
  mutate(prof = gsub("oc.", "", prof)) %>%
  mutate(prof = factor(prof, levels = c("goat.semiaridM","goat.subhumidM","goat.humidM","shp.semiaridM", "shp.subhumidM","shp.humidM")))

# Calculate mean and confidence interval
summary_data <- immDynamics.long %>%
  group_by(Vsched,pV,prof,weeks,Vweeks) %>%
  summarise(
    mean_value = mean(prop_immune),
    lower_ci = quantile(prop_immune, 0.025),
    upper_ci = quantile(prop_immune, 0.975)
...
plot_immVar <- ggplot(summary_data, aes(x = weeks, y = mean_value)) +
  ...
  geom_line(aes(col = factor(pV)), size = 0.7) +
  geom_hline(yintercept = 0.7, linetype = "dashed")+
  facet_grid(Vsched~prof)+ 
  guides(fill = "none")+
  labs(x="Time (years)", y = "Flock proportion immune", col = "pV")+
  coord_cartesian(xlim = c(520,755))+
  scale_x_continuous(breaks = seq(520, 755, by = 52), labels = seq(520, 755, by = 52) / 52)+
  scale_colour_brewer(palette = "RdYlBu")+
  scale_fill_brewer(palette = "RdYlBu")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        legend.position = "top")
...
# with Vweeks
plot_immVarN2 <- ggplot(summary_data,
...
if(plotsave){
  filename_immVar <- "OC_nonseasonalDyn.png"
  ggsave(paste0("plots/applied/",filename_immVar), plot = plot_immVar, width = 25, height = 10, units = "cm")
  
  filename_immVarN <- "OC_nonseasonalDyn-noUncertainty.png"
  ggsave(paste0("plots/applied/",filename_immVarN), plot = plot_immVar.noUncertainty, width = 25, height = 10, units = "cm")
  
  filename_immVarN2 <- "OC_nonseasonalDyn-pV1uncertainty.png"
  ggsave(paste0("plots/applied/",filename_immVarN2), plot = plot_immVarN2, width = 25, height = 10, units = "cm")
  
  # filename_immVarN2 <- "OC_nonseasonalDyn-pV1uncertainty_100.png"
  # ggsave(paste0("plots/applied/",filename_immVarN2), plot = plot_immVarN2, width = 25, height = 10, units = "cm")
}
...
```{r combinedAnalysisSummary}

# load data?
ocOut.nonseasonalSumm <- read.csv("output/Applied/immDyn_nonseasonalSummV12.csv") %>% 
  select(-c("X")) %>%
  mutate(Vsched = "Annual")
ocOut.nonseasonal6Summ <- read.csv("output/Applied/immDyn_nonseasonalSummV6.csv") %>% 
  select(-c("X")) %>%
  mutate(Vsched = "6-monthly")

ocOut.summdf <- ocOut.nonseasonalSumm %>%
  bind_rows(ocOut.nonseasonal6Summ) %>%
  mutate(prof_group = ifelse(grepl("*shp*", prof), "shp", "goat"),
         prof = gsub("oc.", "", prof)) %>%
  mutate(prof = factor(prof, levels = c("goat.semiaridM","shp.semiaridM","goat.subhumidM", "shp.subhumidM","goat.humidM","shp.humidM")))

validImm <- ocOut.summdf %>%
  select(tenyr_growth, starts_with("imm"), Vsched, pV, prof, prof_group) %>%
  gather(key = "stat", value = "value", -c(prof,prof_group, pV, Vsched)) %>%
  mutate(stat = factor(stat, levels = c("imm_V0", "imm_6m", "imm_12m", "imm70_w", "imm70_wAll", "tenyr_growth" )))

# New facet label names for supp variable
supp.labs <- c("imm_V0", "imm_6M", "imm_12M")
names(supp.labs) <- c("imm_V0", "imm_6m", "imm_12m")

pV.labs <- c("pV=0.7", "pV=0.8", "pV=0.9", "pV=1")
names(pV.labs) <- c(0.7,0.8,0.9,1)

plot_imm <- ggplot(validImm %>% filter(stat %in% c("imm_V0", "imm_6m", "imm_12m")), 
                   aes(x = prof, y = value, col = Vsched))+ 
  geom_boxplot(alpha = 0.5, fill = "grey", outlier.shape = NA) +
  geom_hline(yintercept = 0.7, linetype = "dashed")+
  facet_grid(pV~stat,
             labeller = labeller(stat = supp.labs, pV = pV.labs))+
  labs(x = "Flock profile", y="Flock proportion immune", col = "Vaccination Frequency")+
...
imm70_alt <- ggplot(validImm %>% filter(stat %in% c("imm70_w")), 
       aes(x = value, y=Vsched, col = prof))+ #, col = peakmnth
  geom_boxplot(alpha = 0.5, fill = "grey")+
  facet_grid(~pV)+
...
filename_imm <- "OC_nonseasonalImm.png"
# filename_imm <- "OC_nonseasonalImm100.png"
filename_imm70s_alt <- "OC_nonseasonalImm70s_alt.png"

if(plotsave){
  ggsave(paste0("plots/applied/",filename_imm), plot = plot_imm, width = 10, height = 15, units = "cm")
...
validImm_summary <- validImm %>% group_by(Vsched, stat, pV, prof) %>%
  summarise(mean = mean(value) %>% round(.,digits = 2),
            median = median(value) %>% round(.,digits = 2), 
            min = min(value) %>% round(.,digits = 2), 
            max = max(value) %>% round(.,digits = 2)
...
if(plotsave){
  write_csv(pvir70_summary, file = "tables/applied/nonseasonal_pvir70.csv")
  write_csv(pvir70_summary, file = "tables/applied/nonseasonal_pvir70_100.csv")
}
