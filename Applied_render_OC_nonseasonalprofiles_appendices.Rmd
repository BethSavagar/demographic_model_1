...
immDynamics.12 <- read_csv("output/Applied/immDyn_nonseasonalV12.csv") %>% 
  select(-c(`...1`)) %>%
  mutate(Vsched = "Annual")

immDynamics.6 <- read_csv("output/Applied/immDyn_nonseasonalV6.csv") %>% 
  select(-c(`...1`)) %>%
...
plot_imm <- ggplot(validImm %>% filter(stat %in% c("imm_V0", "imm_6m", "imm_12m")), 
  ...
  geom_hline(yintercept = 0.7, linetype = "dashed")+
  facet_grid(pV~stat,
             labeller = labeller(stat = supp.labs))+
